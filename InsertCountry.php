...
<body>
    ...
    <div id="content">
        ...
        ?>
        <div id="section">
            <h2>Datos para añadir nuevo País</h2>
            <form action="InsertCountry.php" method="POST">
                <label for="country_id">ID Country:</label>
                <input type="text" name="country_id" id="country_id" required maxlength="2" ><br><br>

                <label for="country_name">Nombre de País:</label>
                <input type="text" name="country_name" id="country_name" required maxlength="40"
                    placeholder="<?php
                ...
                <?php foreach ($regions as $region) : ?>
						<option value="<?php echo htmlspecialchars($region->region_id); ?>"><?php echo htmlspecialchars($region->region_name); ?></option>
...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Guardar el país a la base de datos
        $country->save();  // INSERT / UPDATE
        echo "<script>alert('País creado con éxito.'); window.location.href='countries.php';</script>";
    }
} catch (\Exception $e) {
